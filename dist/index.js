    ...
    try {
      ...
      if (options.cache === true && !isCached) {
        if (!fs.existsSync(pathe.dirname(cachedFilePath))) {
          await fsp.mkdir(pathe.dirname(cachedFilePath), {
            recursive: true
          });
        }
  ...
  return {
    ...
    async closeBundle() {
      if (publicDir && options.includePublic) {
        ...
        const includedFiles = getFilesToProcess(allFiles, (path) => filename(path) + pathe.extname(path));
        if (allFiles.length > 0)
          await ensureCacheDirectoryExists();
        const handles = allFiles.map(async (publicFilePath) => {
          const filePath = publicFilePath.replace(publicDir + pathe.sep, "");
          const fullFilePath = pathe.join(rootConfig.root, outputPath, filePath);
